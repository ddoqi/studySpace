    ...
    const temp_html = `<div class="card commentCard">
          <div class="card-body">
              ...
              <div class="${isOwner ? "updateBtns" : "noDisplay"}">
                ...
                <button type="button" onclick="pageMoveAndSearchAndCommentsSearch(${cmtObj.createdAt})" > 해당 게시물 이동 </button>
              </div>            
            </div>
     </div>`;
...
// 페이지 이동 , 
// 버튼 클릭 시 넘겨받은 ${cmtObj.createdAt} 로 게시물 조회
// 그 ${cmtObj.createdAt} 가지고 댓글 불러오기

export const pageMoveAndSearchAndCommentsSearch = async (createAtId) => {
  window.location.replace("#boardCommentsPage");
  
  // db 연결작업중이고 where문 어떻게 걸어와야될지 작업하다 스탑한 상태
 
  // const commentsdRef = collection(dbService, "comments");
  // const q = query(commentsdRef, where("createdAt", "==", createAtId));
  // const querySnapshot = await getDocs(q);
  ...
  const boardCommentsList = document.getElementById("boardCommentsList");
  // const currentUid = authService.currentUser.uid;
  ...
  BoardcmtObjList.forEach((cmtObj) => {
    const temp_html = `
    <div>글내용 : <div id="textContent">${cmtObj.text}</div></div>
    <div>작성자 : <div id="userName">${cmtObj.nickname}</div></div>
    <div>creatAt_게시물num : <div id="boardNum">${cmtObj.createdAt}</div></div>
    <hr/>
    <h2>해당 게시물 댓글</h2>
    <button type="button" onclick="targetCommentsSearch(${cmtObj.createdAt})">해당 게시물 댓글보기</button>
    <div id="target_comments"></div>
    <hr/>
    <div>댓글 작성란 <input type="text" id="targetCommentsText"></div>
    <div>댓글작성자 id<div>${authService.currentUser.uid}</div></div>
    <div>댓글타겟게시물번호 : <div class="target_num">${cmtObj.createdAt}</div></div>
    <button type="button" onclick="saveTargetComments(${cmtObj.createdAt})">댓글 저장</button>
    `;

    const div = document.createElement("div");
    div.classList.add("parentId");
    div.innerHTML = temp_html;
    boardCommentsList.appendChild(div);
  });
};


//댓글 저장
//처음부터 cmtObj.createdAt로 넘겨받은 걸 활용하기 -> target_board_num
...
// 댓글 조회메소드(이건 게시물 불러올때 251번 라인에 추가할 것
 // id가 target_comments에 댓글들 내용이 담겨야함
